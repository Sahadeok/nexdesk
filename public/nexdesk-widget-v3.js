...
;(function() {
  ...
  style.textContent = `
    ...
    .nd-title { font-size: 14px; font-weight: 700; color: #e2e8f0; }
    .nd-subtitle { font-size: 11px; color: #64748b; margin-top: 1px; }
    .nd-close {
  ...
  function sendToWatcher(events) {
    // Send to app-watcher (Support 3.0)
    ...
    // Also send to session-events (legacy — so all dashboards see it)
    const sessionEvents = events.map(e => ({
      session_id:  SESSION_ID,
      event_type:  e.event_type || 'api_failure',
      endpoint:    e.endpoint || e.page,
    ...
    fetch(`${NEXDESK_URL}/api/session-events`, {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
