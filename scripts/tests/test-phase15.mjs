...
// removed redundant env import
import { aiWriteCR, analyzeRisk, aiGenerateTechnicalDesign, generateCRNumber } from './lib/crEngine.js'
...
async function runTests() {
  ...
  // ── TEST 5: CR NUMBER GENERATION ──────────────────────────────────
  console.log('\n🔢 TEST 5: CR Number Generation...');
  try {
    const crNum = await generateCRNumber(supabase);
    console.log('✅ CR NUMBER GENERATED:', crNum);
    if (!crNum.startsWith('CR-2026-')) throw new Error('Invalid CR number format');
  } catch (e) {
