...
export default function L2Dashboard() {
  ...
  async function loadTickets() {
    ...
    if (error) { console.error('L2 query error:', error); return }
    
    // Filter out resolved/closed in JavaScript — 100% reliable
    const active = (data || []).filter(t => t.status !== 'resolved' && t.status !== 'closed')
    setTickets(active)
  }

  const l2T  = tickets.filter(t => t.assigned_team === 'L2')
  const escT = tickets.filter(t => t.escalated_to_l2 === true)
  const devT = tickets.filter(t => t.assigned_team === 'DEVELOPER')
  const shown = tab === 'l2' ? l2T : tab === 'escalated' ? escT : tab === 'developer' ? devT : tickets

  const stats = {
    l2:        l2T.length,
    escalated: escT.length,
    developer: devT.length,
    ...
    <div style={{ minHeight:'100vh', background:'#0a0e1a', fontFamily:"'DM Sans',sans-serif", color:'#e2e8f0' }}>
      ...
      <div style={{ maxWidth:1300, margin:'0 auto', padding:'28px 24px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:14, marginBottom:24 }}>
          {[
            ['⚠️', 'L2 Queue',         stats.l2,        '#8b5cf6', '#2e1065'],
            ['🔺', 'Escalated from L1', stats.escalated, '#f97316', '#431407'],
            ['👨‍💻', 'Dev Assigned',       stats.developer, '#06b6d4', '#083344'],
            ['🔴', 'Critical',           stats.critical,  '#ef4444', '#450a0a'],
        ...
        {stats.critical > 0 && (
          <div style={{ background:'#450a0a', border:'1px solid #ef444440', borderRadius:10, padding:'12px 16px', marginBottom:18, display:'flex', alignItems:'center', gap:10 }}>
            ...
            <span style={{ color:'#fca5a5', fontSize:13, fontWeight:500 }}>
              <strong>{stats.critical} CRITICAL</strong> ticket{stats.critical > 1 ? 's' : ''} need immediate attention!
            </span>
          </div>
        )}

        {/* Tabs */}
        <div style={{ display:'flex', gap:2, marginBottom:16, background:'#111827', borderRadius:12, padding:4, border:'1px solid #1f2d45', width:'fit-content' }}>
          {[
            ['all',       '📋 All Active', tickets.length],
            ['l2',        '⚠️ L2 Queue',   stats.l2],
            ['escalated', '🔺 Escalated',  stats.escalated],
            ['developer', '👨‍💻 Developer',  stats.developer],
          ].map(([val, label, count]) => (
            <button key={val} onClick={() => setTab(val)} style={{ padding:'8px 16px', borderRadius:9, fontSize:13, cursor:'pointer', border:'none', fontFamily:"'DM Sans',sans-serif", fontWeight:tab===val?600:400, background:tab===val?'#1a2236':'transparent', color:tab===val?'#e2e8f0':'#64748b', transition:'all 0.2s' }}>
              {label}&nbsp;
              <span style={{ fontSize:11, padding:'1px 6px', borderRadius:10, background:tab===val?'#3b82f620':'#1f2d45', color:tab===val?'#60a5fa':'#475569' }}>
                {count}
              </span>
            </button>
          ))}
        ...
        <div style={{ display:'flex', justifyContent:'flex-end', marginBottom:12 }}>
          <button onClick={loadTickets} style={{ padding:'6px 14px', borderRadius:20, fontSize:12, border:'1px solid #1f2d45', background:'transparent', color:'#475569', cursor:'pointer' }}>
            🔄 Refresh
          </button>
        </div>
        ...
        <div style={{ background:'#111827', border:'1px solid #1f2d45', borderRadius:16, overflow:'hidden' }}>
          ...
          {shown.length === 0 ? (
            <div style={{ padding:48, textAlign:'center' }}>
              ...
              <button onClick={() => setTab('all')} style={{ padding:'8px 20px', background:'#1a2236', border:'1px solid #3b82f640', color:'#60a5fa', borderRadius:8, cursor:'pointer', fontSize:13 }}>
                👆 Switch to All Active
              </button>
            </div>
          ) : (
              ...
              <table style={{ width:'100%', borderCollapse:'collapse' }}>
                <thead>
                  <tr style={{ background:'#0a0e1a' }}>
                    {['Ticket #','Title','Category','Priority','Status','Team','SLA','Actions'].map(h => (
                  ...
                  {shown.map(t => {
                    ...
                    return (
                      <tr key={t.id} className="trow" onClick={() => router.push(`/tickets/${t.id}`)} style={{ borderTop:'1px solid #1f2d45', transition:'background 0.15s' }}>
                        ...
                        <td style={{ padding:'11px 14px' }}><span style={{ fontSize:11, padding:'3px 7px', borderRadius:5, background:tc.bg, color:tc.c, fontWeight:600 }}>{t.assigned_team}</span></td>
...
function Loader() {
  return (
    <div style={{ minHeight:'100vh', background:'#0a0e1a', display:'flex', alignItems:'center', justifyContent:'center' }}>
      <div style={{ width:40, height:40, borderRadius:'50%', border:'3px solid #1f2d45', borderTopColor:'#8b5cf6', animation:'spin 0.7s linear infinite' }}/>
