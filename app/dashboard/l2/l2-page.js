    ...
    <div style={{minHeight:'100vh',background:'#0a0e1a',fontFamily:"'DM Sans',sans-serif",color:'#e2e8f0'}}>
      ...
      <div style={{background:'#111827',borderBottom:'1px solid #1f2d45',padding:'0 28px',height:60,display:'flex',alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,zIndex:100}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:12}}>
          <span style={{padding:'4px 10px',borderRadius:6,fontSize:11,fontWeight:600,background:'#2e1065',color:'#a78bfa',border:'1px solid #8b5cf640'}}>{profile?.role==='ADMIN'?'ADMIN':'L2 AGENT'}</span>
      ...
      <div style={{maxWidth:1300,margin:'0 auto',padding:'28px 24px'}}>
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:14,marginBottom:24}}>
          {[['⚠️','L2 Queue',stats.l2,'#8b5cf6','#2e1065'],['🔺','Escalated',stats.escalated,'#f97316','#431407'],['👨‍💻','Developer',stats.developer,'#06b6d4','#083344'],['🔴','Critical',stats.critical,'#ef4444','#450a0a']].map(([icon,label,val,color,bg],i)=>(
        ...
        {stats.critical > 0 && (
          <div style={{background:'#450a0a',border:'1px solid #ef444440',borderRadius:10,padding:'12px 16px',marginBottom:18,display:'flex',alignItems:'center',gap:10}}>
            ...
            <span style={{color:'#fca5a5',fontSize:13,fontWeight:500}}><strong>{stats.critical} CRITICAL</strong> ticket{stats.critical>1?'s':''} need immediate attention!</span>
          </div>
        )}

        <div style={{display:'flex',gap:2,marginBottom:12,background:'#111827',borderRadius:12,padding:4,border:'1px solid #1f2d45',width:'fit-content'}}>
          {[['all','📋 All Active',tickets.length],['l2','⚠️ L2 Queue',stats.l2],['escalated','🔺 Escalated',stats.escalated],['developer','👨‍💻 Developer',stats.developer]].map(([val,label,count])=>(
            <button key={val} onClick={()=>setTab(val)} style={{padding:'8px 16px',borderRadius:9,fontSize:13,cursor:'pointer',border:'none',fontFamily:"'DM Sans',sans-serif",fontWeight:tab===val?600:400,background:tab===val?'#1a2236':'transparent',color:tab===val?'#e2e8f0':'#64748b'}}>
        ...
        <div style={{background:'#111827',border:'1px solid #1f2d45',borderRadius:16,overflow:'hidden'}}>
          ...
          {shown.length===0?(
            <div style={{padding:48,textAlign:'center'}}>
              ...
              <p style={{color:'#475569'}}>No tickets in this queue</p>
              <p style={{color:'#334155',fontSize:12,marginTop:4}}>👆 Click "All Active" tab above to see all tickets</p>
            </div>
          ):(
            <div style={{overflowX:'auto'}}>
              <table style={{width:'100%',borderCollapse:'collapse'}}>
                <thead><tr style={{background:'#0a0e1a'}}>
                  {['Ticket #','Title','Category','Priority','Status','Team','SLA','AI Reason','Actions'].map(h=>(
