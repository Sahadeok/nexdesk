...
export default function TicketDetail({ params }) {
  ...
  const [action,   setAction]   = useState('')  // escalate | resolve | assign_dev
  ...
  async function loadTicket() {
    const { data } = await supabase.from('tickets').select('*, categories(name,icon,code), profiles!tickets_created_by_fkey(full_name,email)').eq('id', params.id).single()
    if (data) setTicket(data)
  }
  async function loadComments() {
    const { data } = await supabase.from('ticket_comments').select('*, profiles(full_name,email,role)').eq('ticket_id', params.id).order('created_at', { ascending: true })
    if (data) setComments(data)
  }
  async function loadHistory() {
    const { data } = await supabase.from('ticket_history').select('*, profiles(full_name,email)').eq('ticket_id', params.id).order('changed_at', { ascending: false })
  ...
  async function postComment(e) {
    ...
    await supabase.from('ticket_comments').insert({ ticket_id: params.id, user_id: user.id, comment_text: comment.trim(), is_internal: isInternal })
    await supabase.from('ticket_history').insert({ ticket_id: params.id, changed_by: user.id, field_changed: 'comment', note: isInternal ? 'Internal note added' : 'Comment added' })
    setComment(''); await loadComments(); await loadHistory()
    setPosting(false)
  }

  async function doAction() {
    if (!actionNote.trim() && action !== 'resolve') { setMsg('Please enter a reason/note.'); return }
    setSaving(true); setMsg('')
    const { data: { user } } = await supabase.auth.getUser()
    try {
      if (action === 'escalate') {
        await supabase.from('tickets').update({ assigned_team:'L2', escalated_to_l2:true, escalation_reason:actionNote, status:'escalated', updated_at:new Date().toISOString() }).eq('id', params.id)
        await supabase.from('ticket_history').insert({ ticket_id:params.id, changed_by:user.id, field_changed:'assigned_team', old_value:'L1', new_value:'L2', note:'Escalated to L2: '+actionNote })
        setMsg('✅ Ticket escalated to L2 team')
      } else if (action === 'resolve') {
        const note = actionNote.trim() || 'Issue resolved'
        await supabase.from('tickets').update({ status:'resolved', resolution_notes:note, sla_resolve_met:new Date() <= new Date(ticket.sla_resolve_due), resolved_at:new Date().toISOString(), updated_at:new Date().toISOString() }).eq('id', params.id)
        await supabase.from('ticket_history').insert({ ticket_id:params.id, changed_by:user.id, field_changed:'status', old_value:ticket.status, new_value:'resolved', note:'Resolved: '+note })
        setMsg('✅ Ticket resolved')
      } else if (action === 'assign_dev') {
        await supabase.from('tickets').update({ assigned_team:'DEVELOPER', assigned_dev_team:actionNote, assigned_dev_reason:actionNote, status:'in_progress', updated_at:new Date().toISOString() }).eq('id', params.id)
        await supabase.from('ticket_history').insert({ ticket_id:params.id, changed_by:user.id, field_changed:'assigned_team', old_value:ticket.assigned_team, new_value:'DEVELOPER', note:'Assigned to developer: '+actionNote })
        setMsg('✅ Assigned to Developer team')
      }
      await loadTicket(); await loadHistory(); setAction(''); setActionNote('')
    } catch(err) { setMsg('❌ Error: '+err.message) }
    setSaving(false)
  }

  if (loading) return <Loader />
  if (!ticket) return <div style={{minHeight:'100vh',background:'#0a0e1a',display:'flex',alignItems:'center',justifyContent:'center',color:'#ef4444',fontFamily:"'DM Sans',sans-serif"}}>Ticket not found</div>
  ...
  const teamColor = ticket.assigned_team==='L2'?{c:'#a78bfa',bg:'#2e1065'}:ticket.assigned_team==='DEVELOPER'?{c:'#06b6d4',bg:'#083344'}:{c:'#60a5fa',bg:'#1e3a5f'}
    ...
    <div style={{minHeight:'100vh',background:'#0a0e1a',fontFamily:"'DM Sans',sans-serif",color:'#e2e8f0'}}>
      ...
      <div style={{background:'#111827',borderBottom:'1px solid #1f2d45',padding:'0 24px',height:60,display:'flex',alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,zIndex:100}}>
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          <button onClick={()=>router.back()} style={{background:'transparent',border:'none',color:'#64748b',cursor:'pointer',fontSize:18,padding:'0 4px'}}>←</button>
      ...
      <div style={{maxWidth:1200,margin:'0 auto',padding:'28px 24px'}}>
          ...
          <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:16,flexWrap:'wrap'}}>
            <div style={{flex:1}}>
              <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:10,flexWrap:'wrap'}}>
                ...
                <span style={{fontSize:11,padding:'3px 8px',borderRadius:5,background:teamColor.bg,color:teamColor.c,fontWeight:600}}>{ticket.assigned_team}</span>
              ...
              <h1 style={{fontFamily:"'Syne',sans-serif",fontSize:22,fontWeight:800,marginBottom:8,lineHeight:1.3}}>{ticket.title}</h1>
              <p style={{color:'#64748b',fontSize:13}}>
                Raised by <strong style={{color:'#94a3b8'}}>{ticket.profiles?.full_name||ticket.profiles?.email||'User'}</strong> on {new Date(ticket.created_at).toLocaleString('en-IN',{day:'2-digit',month:'short',year:'numeric',hour:'2-digit',minute:'2-digit'})}
              </p>
            </div>
          </div>
        </div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 340px',gap:20,alignItems:'start'}}>
          ...
          <div>
            ...
            {ticket.ai_routing_reason && (
              <div style={{background:'#0f172a',border:'1px solid #1e293b',borderRadius:12,padding:'14px 18px',marginBottom:16,display:'flex',gap:10}}>
                ...
                <div><div style={{fontSize:11,fontWeight:600,color:'#475569',marginBottom:3}}>AI ROUTING REASON</div><div style={{fontSize:12,color:'#64748b'}}>{ticket.ai_routing_reason}</div></div>
              </div>
            )}

            {/* Action panel for agents */}
            {isAgent && !['resolved','closed'].includes(ticket.status) && (
              <div style={{background:'#111827',border:'1px solid #1f2d45',borderRadius:14,padding:'20px 24px',marginBottom:16,animation:'fadeUp 0.4s 0.15s ease both'}}>
                ...
                {msg && <div style={{padding:'10px 14px',borderRadius:8,marginBottom:14,background:msg.startsWith('✅')?'#052e16':'#450a0a',color:msg.startsWith('✅')?'#6ee7b7':'#fca5a5',fontSize:13,border:`1px solid ${msg.startsWith('✅')?'#10b98130':'#ef444430'}`}}>{msg}</div>}
                <div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:14}}>
                  ...
                  {CAN_ASSIGN_DEV.includes(profile?.role) && (
                    <button onClick={()=>setAction(action==='assign_dev'?'':'assign_dev')} style={{padding:'9px 16px',borderRadius:9,fontSize:13,cursor:'pointer',border:'1px solid #06b6d440',background:action==='assign_dev'?'#083344':'transparent',color:'#22d3ee',fontFamily:"'DM Sans',sans-serif"}}>👨‍💻 Assign to Developer</button>
                  )}
                  {CAN_RESOLVE.includes(profile?.role) && (
                ...
                {action && (
                  <div style={{background:'#0a0e1a',borderRadius:10,padding:16,border:'1px solid #1f2d45'}}>
                    ...
                    <textarea className="inp" value={actionNote} onChange={e=>setActionNote(e.target.value)} placeholder={action==='escalate'?'e.g. API error confirmed, needs code fix':action==='assign_dev'?'e.g. Backend team — database query error':'e.g. Resolved by resetting user account'}
            ...
            {/* Comments */}
            <div style={{background:'#111827',border:'1px solid #1f2d45',borderRadius:14,overflow:'hidden',animation:'fadeUp 0.4s 0.2s ease both'}}>
              ...
              <div style={{padding:'16px 24px'}}>
                {comments.length===0 && <p style={{color:'#334155',fontSize:13,padding:'8px 0'}}>No comments yet. Be the first to comment.</p>}
                {comments.map(c=>(
                  <div key={c.id} style={{marginBottom:16,paddingBottom:16,borderBottom:'1px solid #1f2d45'}}>
                    <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:6}}>
                      ...
                      <span style={{fontSize:13,fontWeight:500,color:'#e2e8f0'}}>{c.profiles?.full_name||c.profiles?.email?.split('@')[0]||'User'}</span>
                      <span style={{fontSize:11,color:'#334155',padding:'1px 6px',borderRadius:4,background:c.is_internal?'#451a03':'transparent',color:c.is_internal?'#fb923c':'#475569'}}>{c.is_internal?'🔒 Internal':''}</span>
                ...
                <form onSubmit={postComment} style={{marginTop:8}}>
                  <textarea className="inp" value={comment} onChange={e=>setComment(e.target.value)} placeholder="Write a comment..." style={{width:'100%',padding:'10px 12px',background:'#0a0e1a',border:'1px solid #1f2d45',borderRadius:10,color:'#e2e8f0',fontFamily:"'DM Sans',sans-serif",fontSize:13,outline:'none',resize:'vertical',minHeight:80,marginBottom:10}}/>
          ...
          {/* Right: Ticket Details + History */}
          <div>
            {/* Details card */}
            <div style={{background:'#111827',border:'1px solid #1f2d45',borderRadius:14,padding:'20px',marginBottom:14,animation:'fadeUp 0.4s 0.1s ease both'}}>
              ...
              {[
                ...
                <div key={k} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'8px 0',borderBottom:'1px solid #0f172a'}}>
                  ...
                  <span style={{fontSize:12,color:'#e2e8f0',fontWeight:500,textAlign:'right',maxWidth:180}}>{v}</span>
                </div>
              ))}
              {ticket.resolution_notes && (
                <div style={{marginTop:12,padding:'10px 12px',background:'#052e16',borderRadius:8,border:'1px solid #10b98130'}}>
                  <div style={{fontSize:11,color:'#10b981',fontWeight:600,marginBottom:4}}>✅ Resolution</div>
                  <div style={{fontSize:12,color:'#6ee7b7'}}>{ticket.resolution_notes}</div>
                </div>
              )}
            </div>

            {/* History */}
            <div style={{background:'#111827',border:'1px solid #1f2d45',borderRadius:14,padding:'20px',animation:'fadeUp 0.4s 0.2s ease both',maxHeight:400,overflow:'auto'}}>
              <div style={{fontSize:12,fontWeight:600,color:'#475569',textTransform:'uppercase',letterSpacing:'0.5px',marginBottom:14}}>📋 Activity History</div>
              {history.length===0 ? <p style={{fontSize:12,color:'#334155'}}>No history yet</p> : (
                history.map((h,i)=>(
                  <div key={h.id} style={{display:'flex',gap:10,marginBottom:12,paddingBottom:12,borderBottom:i<history.length-1?'1px solid #0f172a':'none'}}>
                    <div style={{width:24,height:24,borderRadius:'50%',background:'#1e293b',display:'flex',alignItems:'center',justifyContent:'center',fontSize:10,flexShrink:0,marginTop:2}}>
                      {h.field_changed==='status'?'🔄':h.field_changed==='assigned_team'?'👤':h.field_changed==='comment'?'💬':'📝'}
                    </div>
                    <div>
